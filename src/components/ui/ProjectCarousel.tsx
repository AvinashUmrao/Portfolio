...
const ProjectCard = ({ project }: { project: Project }) => {
  ...
  return (
    <div className="flex-none px-2 w-[260px] xs:w-[300px] sm:w-[320px] md:w-[360px]">
      <Card className="h-full bg-card backdrop-blur-sm border-border hover:bg-accent/5 transition-colors overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 text-xs bg-primary/10 text-secondary-text rounded-full"
              >
                {tag}
              </span>
            ))}
